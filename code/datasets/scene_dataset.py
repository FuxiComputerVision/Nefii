...
class SceneDataset(torch.utils.data.Dataset):
    ...
    def batch_ray_sample(self, s_uv_btach):
        B, S, _ = s_uv_btach.shape
        s_uv_btach = s_uv_btach.reshape(B * S, 2)
        sr_uv = self.ray_sample(s_uv_btach)  # B*S X R x 2
        sr_uv_batch = sr_uv.reshape(B, S, -1, 2)  # B x S x R x 2
        return sr_uv_batch
    ...
    def change_sampling_rays(self, sampling_size):
        if sampling_size == -1:
            self.sampling_rays = None
        else:
            self.sampling_rays = torch.rand((sampling_size, 2)) - 0.5
    ...
    def scatter_sampling_idx(self, rank, world_size):
        if self.sampling_idx is not None:
            sub_size = self.sampling_idx.shape[0] // world_size
            if rank < world_size - 1:
                self.sampling_idx = self.sampling_idx[rank * sub_size: rank * sub_size + sub_size]
            else:
                self.sampling_idx = self.sampling_idx[rank * sub_size:]

    def scatter_sampling_idx_patch(self, rank, world_size, N_patch, r_patch=1):
        if self.sampling_idx is not None:
            offsets_len = 4 * r_patch * r_patch
            select_inds = self.sampling_idx.reshape(-1, offsets_len)

            sub_size = select_inds.shape[0] // world_size
            if rank < world_size - 1:
                select_inds_local = select_inds[rank * sub_size: rank * sub_size + sub_size]
            else:
                select_inds_local = select_inds[rank * sub_size:]

            self.sampling_idx = select_inds_local.reshape(-1)
