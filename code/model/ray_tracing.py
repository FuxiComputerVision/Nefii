...
class RayTracing(nn.Module):
    def __init__(
        ...
        # self.max_sdf_input = 1600000
        self.max_sdf_input = 100000
    ...
    def ray_sampler(self, sdf, cam_loc, object_mask, ray_directions, sampler_min_max, sampler_mask):
        ...
        sdf_val_all = []
        for pnts in torch.split(points.reshape(-1, 3), self.max_sdf_input, dim=0):
        ...
        if n_rootfind_pts > 0:
            ...
            # z_pred_rootfind = self.secant(sdf_low, sdf_high, z_low, z_high, cam_loc_rootfind, ray_directions_rootfind,
            #                                 sdf)
    ...
    def minimal_sdf_points(self, num_pixels, sdf, cam_loc, ray_directions, mask, min_dis, max_dis):
        ...
        mask_sdf_all = []
        for pnts in torch.split(points, self.max_sdf_input, dim=0):
...
if __name__ == "__main__":
    ...
    from model.implicit_differentiable_renderer import ImplicitNetwork
    import time
    from torch.profiler import profile, record_function, ProfilerActivity

    conf = ConfigFactory.parse_file(conf_file).get_config('model')

    # init model
    implicit_network = ImplicitNetwork(conf.get_int('feature_vector_size', 0), **conf.get_config('implicit_network'))
    ...
    for k, v in geometry.items():
        if 'implicit_network' in k:
            index = k.index('implicit_network')
            new_key = k[index + len('implicit_network.'):]
            geometry_part[new_key] = v
    geometry = geometry_part
    print(geometry.keys())
    model_dict = implicit_network.state_dict()
    model_dict.update(geometry)
    implicit_network.load_state_dict(model_dict)

    # random generate camera location
    elevation = torch.rand(ray_number) * torch.pi * 0.5
    azimuth = torch.rand(ray_number) * torch.pi * 2

    z = elevation.cos() * dist
    y = elevation.sin() * elevation.sin() * dist
    x = elevation.sin() * elevation.cos() * dist
    cam_loc = torch.stack([x, y, z], dim=-1)  # Bx3

    raydir = 0 - cam_loc  # Bx3
    raydir = raydir / torch.norm(raydir, dim=-1, keepdim=True)
    raydir = raydir[:, None, :]  # Bx1x3

    object_mask = torch.ones(cam_loc.shape[0]).to(cam_loc.device) > 0

    # gpu
    implicit_network = implicit_network.cuda()
    ray_tracer = ray_tracer.cuda()
    cam_loc = cam_loc.cuda()
    raydir = raydir.cuda()
    object_mask = object_mask.cuda()

    def sdf(x):
        with record_function("sdf"):
            # s = implicit_network(x)[:, 0]
    ...
    prof.export_chrome_trace(r"D:\Cloud_buffer\trace.json")
