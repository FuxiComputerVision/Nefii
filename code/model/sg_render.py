...
#######################################################################################################
def compute_envmap(lgtSGs, H, W, upper_hemi=False, log=True, coordinate_type="mitsuba", envmap_type="sg"):
    if envmap_type == "constant":
        return compute_envmap_2d(lgtSGs, H, W)

    if coordinate_type == "mitsuba":
        # exactly same convetion as Mitsuba, check model/sg_envmap_convention.png
        if upper_hemi:
        ...
        viewdirs = torch.stack([torch.cos(theta) * torch.sin(phi), torch.cos(phi), torch.sin(theta) * torch.sin(phi)],
                               dim=-1)    # [H, W, 3]
    elif coordinate_type == "blender":
        # exactly same convetion as blender, check model/blender_envmap_convention.png
        if upper_hemi:
            phi, theta = torch.meshgrid(
                [torch.linspace(0., np.pi / 2., H), torch.linspace(1.0 * np.pi, -1.0 * np.pi, W)])
        else:
            phi, theta = torch.meshgrid([torch.linspace(0., np.pi, H), torch.linspace(1.0 * np.pi, -1.0 * np.pi, W)])

        viewdirs = torch.stack([torch.cos(theta) * torch.sin(phi), torch.sin(theta) * torch.sin(phi), torch.cos(phi)],
                               dim=-1)  # [H, W, 3]
    if log:
...
def compute_envmap_2d(lgtMap, H, W):
    """
    :param lgtMap: MxMx3
    :return HxWx3
    """
    lgtMap = lgtMap.permute(2, 0, 1)  # 3xMxM
    envmap = torch.nn.functional.interpolate(lgtMap[None], size=(H, W), mode='bilinear').squeeze(0)  # 3xHxW
    envmap = envmap.permute(1, 2, 0)  # HxWx3
    return envmap
